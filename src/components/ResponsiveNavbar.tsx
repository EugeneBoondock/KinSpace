...
export default function ResponsiveNavbar() {
    ...
    return (
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-brand-accent3 border-opacity-50 px-4 sm:px-10 py-3 relative z-30 bg-[#eedfc8] text-[#2A4A42]">
        ...
        <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
          <div className="w-24 h-10 bg-[#2A4A42]/10 rounded-full animate-pulse"></div>
          <div className="w-24 h-10 bg-[#2A4A42]/10 rounded-full animate-pulse"></div>
        </div>
      </header>
    );
  ...
  const handleLogout = async () => {
    try {
      ...
      // setUser will be handled by onAuthStateChange
  ...
  return (
    <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-brand-accent3 border-opacity-50 px-4 sm:px-10 py-3 relative z-30 bg-[#eedfc8] text-[#2A4A42]">
      ...
      <nav className="hidden md:flex flex-1 justify-end gap-8 items-center">
        ...
        {user && (
          <>
            <Link href={`/profile/${user.id}`} className="text-[#2A4A42] text-sm font-medium leading-normal hover:text-brand-accent1 transition-colors duration-200 px-3 py-1 rounded-md hover:bg-[#2A4A42]/10">
              Profile
            </Link>
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-50 bg-[#eedfc8] flex flex-col items-end p-4 animate-fade-in">
          ...
          <nav className="w-full flex flex-col gap-2">
            ...
            {user && (
              <>
                <Link href={`/profile/${user.id}`} className="block px-4 py-3 text-[#2A4A42] hover:bg-brand-accent3 hover:text-white rounded text-lg font-medium" onClick={() => setOpen(false)}>Profile</Link>
