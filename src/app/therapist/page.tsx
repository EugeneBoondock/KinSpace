...
const [user, setUser] = useState<UserProfile | null>(null);
  ...
  return (
    <div className="relative flex size-full min-h-screen flex-col bg-[#201913] dark group/design-root overflow-x-hidden" style={{ fontFamily: 'Plus Jakarta Sans, Noto Sans, sans-serif' }}>
      <div className="layout-container flex h-full grow flex-col">
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#433428] px-10 py-3">
          ...
          <div className="flex flex-1 justify-end gap-8">
            <div className="flex items-center gap-9">
              ...
              <a className="text-white text-sm font-medium leading-normal" href="#">Conditions</a>
              <a className="text-white text-sm font-medium leading-normal" href="#">Groups</a>
            ...
            <button className="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 bg-[#433428] text-white gap-2 text-sm font-bold leading-normal tracking-[0.015em] min-w-0 px-2.5"><div className="text-white" data-icon="Bell" data-size="20px" data-weight="regular"><svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" fill="currentColor" viewBox="0 0 256 256"><path d="M221.8,175.94C216.25,166.38,208,139.33,208,104a80,80,0,1,0-160,0c0,35.34-8.26,62.38-13.81,71.94A16,16,0,0,0,48,200H88.81a40,40,0,0,0,78.38,0H208a16,16,0,0,0,13.8-24.06ZM128,216a24,24,0,0,1-22.62-16h45.24A24,24,0,0,1,128,216ZM48,184c7.7-13.24,16-43.92,16-80a64,64,0,1,1,128,0c0,36.05,8.28,66.73,16,80Z"></path></svg></div></button>
        ...
        <div className="px-40 flex flex-1 justify-center py-5">
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex items-end gap-3 p-4 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                  ...
                  <div className={`flex flex-1 flex-col gap-1 items-${msg.sender === 'user' ? 'end' : 'start'}`}> 
                    <p className="text-[#c2ab99] text-[13px] font-normal leading-normal max-w-[360px] {msg.sender === 'user' ? 'text-right' : ''}">{msg.name}</p>
                    <p className={`text-base font-normal leading-normal flex max-w-[360px] rounded-xl px-4 py-3 ${msg.sender === 'user' ? 'bg-[#cd7731]' : 'bg-[#433428]'} text-white`}>
                      {msg.text}
                    </p>
                  </div>
                  {msg.sender === 'user' && (
            ...
            <div className="flex justify-center">
              <div className="flex flex-1 gap-3 flex-wrap px-4 py-3 max-w-[480px] justify-center">
                {quickPrompts.map((prompt) => (
                  <button
                    key={prompt}
                    className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-[#433428] text-white text-sm font-bold leading-normal tracking-[0.015em] grow"
            ...
            <form onSubmit={handleSend} className="flex items-center px-4 py-3 gap-3 @container">
              <label className="flex flex-col min-w-40 h-12 flex-1">
                <div className="flex w-full flex-1 items-stretch rounded-xl h-full">
                  <input
                    ...
                    className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-white focus:outline-0 focus:ring-0 border-none bg-[#433428] focus:border-none h-full placeholder:text-[#c2ab99] px-4 rounded-r-none border-r-0 pr-2 text-base font-normal leading-normal"
                    value={input}
                    onChange={e => setInput(e.target.value)}
                    disabled={loading}
                  />
                  <div className="flex border-none bg-[#433428] items-center justify-center pr-4 rounded-r-xl border-l-0 !pr-2">
                    <div className="flex items-center gap-4 justify-end">
                      <div className="flex items-center gap-1">
                        <button type="button" className="flex items-center justify-center p-1.5" disabled>
                          <div className="text-[#c2ab99]" data-icon="Image" data-size="20px" data-weight="regular">
                        ...
                        <button type="button" className="flex items-center justify-center p-1.5" disabled>
                          <div className="text-[#c2ab99]" data-icon="Microphone" data-size="20px" data-weight="regular">
              ...
              <button type="submit" className="ml-2 px-6 py-2 bg-[#cd7731] text-white font-bold rounded-full hover:bg-[#b95e1e] transition" disabled={loading || !input.trim()}>{loading ? 'Sending...' : 'Send'}</button>
            </form>
            <div className="flex justify-center">
              <div className="flex flex-1 gap-3 flex-wrap px-4 py-3 max-w-[480px] justify-center">
                <button className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-transparent text-white text-sm font-bold leading-normal tracking-[0.015em] grow" disabled>
